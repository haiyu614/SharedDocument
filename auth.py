...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and user.check_password(form.password.data):
            # PyJWT 要求 sub (subject) 为字符串，统一转换
...
def refresh():
    ...
    # 统一使用字符串 identity 避免 PyJWT 对 sub 类型的限制报错
...
def index():
    current_user_id = get_jwt_identity()
    try:
        uid_int = int(current_user_id) if current_user_id is not None else None
    ...
    my_documents = Document.query.filter_by(
        user_id=current_user_id).order_by(Document.upload_date.desc()).all()

    # Shared with me
    shared_shares = DocumentShare.query.filter_by(
    ...
    return render_template('index.html', title='共享文档系统', user=user, my_documents=my_documents, shared_documents=shared_documents, doc_permissions=doc_permissions)

# 诊断用：不加 jwt_required，观察上下文中的身份是否能提取


@auth.route('/index_noguard')
def index_noguard():
    try:
    ...
    return jsonify({
        ...
        'has_refresh_cookie': 'refresh_token_cookie' in request.cookies
    })
